...
long devGpioInitRecord( dbCommon *prec, devGpio_rec_t* pconf ){
  ...
  if( options.size() > 1 || options.empty() ) { // Leds for now have only 1 argument
    std::cerr << prec->name << ": Invalid INP/OUT field: " << ss.str() << "\n"
              << "    Syntax is \"<GPIO>\" - For now leds have only 1 argument" << std::endl;
    return ERROR;
  }
  ...
  try{
    ...
    // one GPIO.
    // Leds doesn't need to wait for Udev
      //GpioManager::instance().waitForUdev( gpioID );

    //GpioManager::instance().setLogic( gpioID, logic ); // Leds doesn't have logic option
...
long devGpioWrite( devGpio_info_t *pinfo ){
  try{
    GpioManager::instance().setValue( pinfo->value );
  } catch( GpioManagerError &e ) {
    strncpy( pinfo->errmsg, e.what(), 255 );
    return ERROR;
  }
...
extern "C" {
  ...
  //----------------------------------------------------------------------------
  void devFileConstRegister( void ) {
  ...
  epicsExportRegistrar( devFileConstRegister );
}
