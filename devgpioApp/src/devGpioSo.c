...
static long devGpioInitRecord_bo( stringoutRecord *prec );
static long devGpioWrite_bo( stringoutRecord *prec );

/*_____ G L O B A L S ________________________________________________________*/
devGpio_dset_t devGpioStringout = {
  6,
  NULL,
  devGpioInit,
...
epicsExportAddress( dset, devGpioStringout );
 ...
 * @return  In case of error return -1, otherwise return 0
 *----------------------------------------------------------------------------*/
long devGpioWrite_bo( stringoutRecord *prec ) {
  devGpio_info_t *pinfo = (devGpio_info_t *)prec->dpvt;
  pinfo->value = prec->val;
